...
with col1:
    ...
    if st.button("Oblicz dla Algorytmu 1"):
        ...
        if plik1 and plik2:
            st.write("Uruchamianie Algorytmu 1...")
            t1=time.time()
            ...
            result1 = subprocess.run(['python', alg1_path, tmp1_path, tmp2_path], capture_output=True, text=True)
            
            if result1.stdout:
                st.session_state.result1 = result1.stdout.strip()
                st.write(f" \n {result1.stdout.strip()}")
            if result1.stderr:
                st.error(f"Błąd Algorytmu 1: {result1.stderr}")
...
with col2:
    ...
    if st.button("Oblicz dla Algorytmu 2"):
        ...
        if plik3:
            ...
            result2 = subprocess.run(['python', alg2_path, tmp3_path], capture_output=True, text=True)
            
            if result2.stdout:
                st.session_state.result2 = result2.stdout.strip()
                st.write(f"Procent podobieństwa do plików w bazie: {result2.stdout.strip()}%")
            #if result2.stderr:
