...
class ReportGenerator:
    def __init__(self):
        ...
        self.tex_pattern = 'report_pattern2.tex'
        self.tex_output_folder = "reports"
        self.tex_png_output_folder = "elements"
        self.tex_out_file_name = "report2.tex"
    ...
    def generate_latex_report(self,data):
        ...
        tex_pattern_path=os.path.join(current_dir,self.tex_pattern)
        
        with open(tex_pattern_path, 'r') as f:
            tex = f.read()
    ...
    def generate_html_report(self,data):
        current_dir = os.path.dirname(os.path.abspath(__file__))
        html_pattern_path=os.path.join(current_dir,self.html_pattern)

        #["24.01.2025","13:34","test.tex","green", "40.0", "123", "54" ],
        with open(html_pattern_path, 'r', encoding='utf-8') as f:
        ...
        print('HTML BODY')
        print(html_body)
        output_html_path=os.path.join(current_dir,self.tex_output_folder)
        with open(os.path.join(output_html_path,self.html_out_file_name), 'w', encoding='utf-8') as f:
            f.write(html_body)
            print('otwarto HTML ############################')
    ...
    def open_html_report(self):
         current_dir = os.path.dirname(os.path.abspath(__file__))
         output_html_path=os.path.join(current_dir,self.tex_output_folder)
         webbrowser.open(os.path.join( output_html_path,self.html_out_file_name))
